...
【更新履歴】
Version 1.4.3
- FIX : ScriptedImporter : Unity6以降でExportを有効にした場合に発生するエラーの修正
- FIX : Unity6000.4以降で表示される警告の修正

Version 1.4.2
- FIX : 現在のRenderPipelineの取得処理の修正
- FIX : DaeExpolter : 最新に修正

Version 1.4.1
- FIX : LowPolyTextureBufferの破棄で例外が発生する問題を修正

Version 1.4.0
- ADD : Unity6  : 対応
- FIX : 最低動作バージョンをUnity2021.3に変更
- FIX : Edit : Space keyで元モデルを表示する編集モードでUnityのMenuが表示される問題の修正
- FIX : Material : voxから読み取り設定するマテリアル設定の修正
- FIX : Prefab : Prefabにする過程で変更したマテリアルが初期化されていた問題の修正
- FIX : Explosion : Preview : Prefab Modeで操作可能に対応、再生スピード修正
- FIX : Explosion : BuiltinRP : Surface Shader版をlegacyとしてShader Graph版を使用することを推奨にして切り替え可能に対応
- FIX : Explosion : マテリアル設定の修正、Color space対応の修正

Version 1.3.4
- FIX : インポート時のメモリ使用量の削減
- FIX : Unity2019.3以前の古い処理の削除

Version 1.3.3
- ADD : Unity2023.2 : 対応
- ADD : ScriptedImporter : 例外発生時に内容のコンソールへの出力を追加
- ADD : Optimize : Ignore Cavity : デフォルト設定を有効から無効に変更
- FIX : Optimize : Ignore Cavity : 有効時に必要な計算を無効時に行わないように対応

Version 1.3.2
- ADD : Unity2023.1 : 対応
- ADD : 設定の変更による自動的な更新を制御するAuto Refreshフラグの追加
- ADD : VoxelChunksObject : Auto Refreshフラグ追加により必要なくなった設定が変更された場合の特殊処理の削除

Version 1.3.1
- FIX : 最低動作バージョンをUnity2019.4に変更

Version 1.3.0
- ADD : 最低動作バージョンをUnity2020.3に変更
- ADD : Unity2022.2 : 対応
- ADD : Chunks : チャンクのピボット指定機能
- ADD : ScriptedImporter : Mesh Modeに内包するModelをすべて読み込むModelsタイプを追加
- FIX : VoxelFrameAnimationObject : MagicaVoxel 0.99.7のAnimationの読み込みの仮対応
- FIX : 巨大なボクセルのインポートでメモリ不足とならないようにメモリ使用量を削減
- FIX : インポートの高速化
- FIX : Configure Disable Face : ShowNotification修正
