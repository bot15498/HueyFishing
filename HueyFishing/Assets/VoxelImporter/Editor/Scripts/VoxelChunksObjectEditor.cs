...
namespace VoxelImporter
{
    ...
    public class VoxelChunksObjectEditor : VoxelBaseEditor
    {
        ...
        {
            EditorGUI.BeginChangeCheck();
            var importScale = EditorGUILayout.Vector3Field("Import Scale", objectTarget.importScale);
            if (EditorGUI.EndChangeCheck())
            {
        ...
        {
            UndoRecordObject("Inspector");
            objectTarget.importScale = scale;
            Refresh();
        }
        protected override void InspectorGUI_ImportSettingsImportOffset()
        {
            EditorGUI.BeginChangeCheck();
            var importOffset = EditorGUILayout.Vector3Field("Import Offset", objectTarget.importOffset);
            if (EditorGUI.EndChangeCheck())
            {
                UndoRecordObject("Inspector");
                objectTarget.importOffset = importOffset;
	            Refresh();
            }
        }
        protected override void InspectorGUI_ImportSettingsExtra()
        {
            ...
            #endregion
            {
                ...
                {
                    EditorGUI.BeginChangeCheck();
                    var chunkSize = EditorGUILayout.Vector3Field("Chunk Size", new Vector3(objectTarget.chunkSize.x, objectTarget.chunkSize.y, objectTarget.chunkSize.z));
                    if (EditorGUI.EndChangeCheck())
                    {
                        UndoRecordObject("Inspector");
                        objectTarget.chunkSize.x = Mathf.RoundToInt(chunkSize.x);
                        objectTarget.chunkSize.y = Mathf.RoundToInt(chunkSize.y);
                        objectTarget.chunkSize.z = Mathf.RoundToInt(chunkSize.z);
                        Refresh();
                    }
                }
                #endregion
            ...
            {
                EditorGUI.BeginChangeCheck();
                var pivotT = EditorGUILayout.Vector3Field("Chunk Pivot T", objectTarget.chunkPivotT);
                if (EditorGUI.EndChangeCheck())
                {
                    UndoRecordObject("Inspector");
                    objectTarget.chunkPivotT = pivotT;
                    Refresh();
                }
            }
            #endregion
