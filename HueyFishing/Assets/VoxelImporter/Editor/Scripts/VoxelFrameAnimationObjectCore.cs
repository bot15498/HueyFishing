...
namespace VoxelImporter
{
    public class VoxelFrameAnimationObjectCore : VoxelBaseCore
    {
        ...
        protected override void CreateOutsideTable()
        {
            if (isFrameDataCreateVoxelTable)
                base.CreateOutsideTable();
        }
        ...
        protected override bool CreateMesh()
        {
            ...
            Action<string> DisplayProgressBar = (info) =>
            {
                if (voxelData.voxels.Length > 10000)
                    EditorUtility.DisplayProgressBar("Create...", string.Format("{0} / {1} {2}", ProgressCount, MaxProgressCount, info), (ProgressCount++ / MaxProgressCount));
            };
            #endregion
                    ...
                    for (int i = 0; i < voxelObject.frames.Count; i++)
                    {
                        ...
                        for (int j = 0; j < voxelData.voxels.Length; j++)
                        {
                            ...
                            if (!paletteTable.ContainsKey(color))
                                paletteTable.Add(color, (byte)paletteTable.Count);
                            voxel.palette = paletteTable[color];
                            voxel.z = (short)(voxel.z + offset);
                            voxels[index++] = voxel;
            ...
            DisplayProgressBar("Create Texture");
            {
            ...
            #region CreateMesh
            DisplayProgressBar("Create Mesh");
            ...
            DisplayProgressBar("Create Material");
            {
