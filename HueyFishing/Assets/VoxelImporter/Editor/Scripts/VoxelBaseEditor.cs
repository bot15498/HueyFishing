    ...
    public abstract class VoxelBaseEditor : Editor
    {
        ...
        protected virtual void InspectorGUI_Refresh()
        {
            EditorGUILayout.BeginHorizontal();
            {
                ...
                EditorGUI.BeginChangeCheck();
                var edit_autoRefresh = EditorGUILayout.ToggleLeft(new GUIContent("Auto Refresh", "Automatically refreshes when settings are changed."), baseTarget.edit_autoRefresh,
                    baseTarget.edit_autoRefresh ? EditorStyles.label : guiStyleRedBold, GUILayout.Width(116f));
                if (EditorGUI.EndChangeCheck())
                {
                    UndoRecordObject("Inspector");
                    baseTarget.edit_autoRefresh = edit_autoRefresh;
                    if (baseTarget.edit_autoRefresh)
                        Refresh();
                }
            }
            {
        ...
        protected void UpdateSilhouetteMeshMesh()
        {
            ...
            if (baseTarget.edit_configureMode == VoxelBase.Edit_ConfigureMode.Material ||
                baseTarget.edit_configureMode == VoxelBase.Edit_ConfigureMode.Disable)
            {
                editorCommon.silhouetteMesh = baseCore.Edit_CreateMesh(baseCore.voxelData.voxels.ToList(), null, true);
            }
        }
                    ...
                    case VoxelBase.Edit_ConfigureMaterialSubMode.Voxel:
                        {
                            ...
                            if (result.HasValue)
                            {
                                editorCommon.cursorMesh = baseCore.Edit_CreateMesh(new List<VoxelData.Voxel>() { new VoxelData.Voxel(result.Value.x, result.Value.y, result.Value.z) });
                            }
                        }
                        break;
        ...
        private void ShowNotification()
        {
            if (baseTarget.edit_configureMode == VoxelBase.Edit_ConfigureMode.Material)
                ...
                SceneView.currentDrawingSceneView.ShowNotification(new GUIContent(string.Format("{0} - {1}", baseTarget.edit_configureDisableMainMode, baseTarget.edit_configureDisableSubMode)));
        }
        ...
        {
            if (!baseTarget.edit_autoRefresh && !force)
                return;
