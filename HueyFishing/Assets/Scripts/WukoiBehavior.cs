...
public class WukoiBehavior : FishMovement
{
    [Header("Visuals")]
    ...
    public FishCatchbar fishCatchbar;

    [Tooltip("Prefab to spawn when a phase threshold is reached.")]
    public GameObject clonePrefab;

    [Tooltip("How many clones to spawn at 33% catch progress.")]
    public int clonesAt33Percent = 3;

    [Tooltip("How many clones to spawn at 66% catch progress.")]
    public int clonesAt66Percent = 5;

    [Header("Clone Ring Spawn")]
    [Tooltip("Desired ring radius from the arena center.")]
    public float cloneSpawnRadius = 5f;

    [Tooltip("Extra padding from arena edges so the ring fully fits.")]
    public float cloneSpawnPadding = 0.5f;

    [Tooltip("Optional angle offset in degrees for rotating the whole clone ring.")]
    public float cloneAngleOffset = 0f;

    [Header("Clone Tracking")]
    ...
    private bool triggered33;
    private bool triggered66;
    ...
    private void Awake()
    {
        ...
        if (fishCatchbar == null)
            fishCatchbar = GetComponent<FishCatchbar>();
    }
    ...
    {
        if (fishCatchbar == null || fishCatchbar.maxCatchBar <= 0)
            return;

        float catchPercent = (float)fishCatchbar.currCatchBar / fishCatchbar.maxCatchBar;

        if (!triggered33 && catchPercent >= 0.33f)
        {
            triggered33 = true;
            SpawnClones(clonesAt33Percent);
        }

        if (!triggered66 && catchPercent >= 0.66f)
        {
            triggered66 = true;
            SpawnClones(clonesAt66Percent);
        }
    }

    private void SpawnClones(int amount)
    {
        if (clonePrefab == null || amount <= 0)
            return;

        Vector3 center = fishSpawnCenter != null
            ? fishSpawnCenter.transform.position
            : arenaCenter;

        center.y = 0f;

        float maxRadiusThatFits =
            Mathf.Min(arenaSize.x * 0.5f, arenaSize.y * 0.5f) - cloneSpawnPadding;

        float spawnRadius = Mathf.Min(cloneSpawnRadius, maxRadiusThatFits);
        spawnRadius = Mathf.Max(0f, spawnRadius);

        for (int i = 0; i < amount; i++)
        {
            Vector3 spawnPos = GetEvenlyDistributedCloneSpawnPoint(center, spawnRadius, i, amount);
            spawnPos.y = 0f;

            GameObject clone = Instantiate(clonePrefab, spawnPos, transform.rotation);
            clone.transform.position = new Vector3(clone.transform.position.x, 0f, clone.transform.position.z);

            spawnedClones.Add(clone);

            WukoiCloneBehavior cloneBehavior = clone.GetComponent<WukoiCloneBehavior>();
            if (cloneBehavior != null)
            {
                cloneBehavior.arenaCenter = new Vector3(arenaCenter.x, 0f, arenaCenter.z);

                if (fishSpawnCenter != null)
                    cloneBehavior.fishSpawnCenter = fishSpawnCenter;

                cloneBehavior.SetQuadrant((WukoiCloneBehavior.CloneQuadrant)(i % 4));
            }
            ...
            if (cloneRb != null)
            {
                ...
                cloneRb.position = new Vector3(cloneRb.position.x, 0f, cloneRb.position.z);
                cloneRb.constraints = RigidbodyConstraints.FreezeRotation | RigidbodyConstraints.FreezePositionY;
            }
        }
    }

    private Vector3 GetEvenlyDistributedCloneSpawnPoint(Vector3 center, float radius, int index, int total)
    {
        if (total <= 0)
            return center;

        float angleStep = 360f / total;
        float angle = cloneAngleOffset + angleStep * index;
    ...
    public void CleanupAllClones()
    {
        for (int i = spawnedClones.Count - 1; i >= 0; i--)
        {
            if (spawnedClones[i] != null)
                Destroy(spawnedClones[i]);
        }

        spawnedClones.Clear();
    }

    public void CleanupNullClonesOnly()
    {
        CleanupMissingCloneReferences();
    }

    public int GetAliveCloneCount()
    {
        CleanupMissingCloneReferences();
        return spawnedClones.Count;
    }

    private void CleanupMissingCloneReferences()
    {
        for (int i = spawnedClones.Count - 1; i >= 0; i--)
        {
            if (spawnedClones[i] == null)
    ...
    private Vector3 GetCenterBiasedPoint()
    {
        ...
        return new Vector3(arenaCenter.x + rx, 0f, arenaCenter.z + rz);
    }
    ...
    private void OnDrawGizmosSelected()
    {
        ...
        Vector3 center = fishSpawnCenter != null ? fishSpawnCenter.transform.position : arenaCenter;
        center.y = 0f;
