...
#if VOXELIMPORTER_ANIMATIONRIGGING
...
namespace VoxelImporter
{
    ...
    {
        private readonly string ContributorAuthoring_Tool = typeof(DaeExporter).Namespace;
        //private readonly string ContributorComments = "https://assetstore.unity.com/packages/tools/animation/very-animation-96826";     //VA
        private readonly string ContributorComments = "https://assetstore.unity.com/packages/tools/modeling/voxel-importer-62914";   //VI
        ...
        public bool Export(string path, List<Transform> transforms, AnimationClip[] clips = null)
        {
            ...
            try
            {
                ...
                string MakeID(UnityEngine.Object o)
                {
                    ...
                    var id = o.GetEntityId().ToString();
#else
                    var id = o.GetInstanceID().ToString(numberFormatInfo);
#endif
                    return id.Replace('-', 'n');
                }
                Mesh MeshFromTransform(Transform t)
                {
                ...
                Material[] MaterialsFromTransform(Transform t)
                {
                    #region SkinnedMeshRenderer
                    {
                        var skinnedMeshRenderer = t.GetComponent<SkinnedMeshRenderer>();
                    ...
                    string ExportTexture(Texture tex)
                    {
                        ...
                        if (AssetDatabase.Contains(tex) && AssetDatabase.IsMainAsset(tex))
                        {
                            if (path.StartsWith(Application.dataPath))
                            {
                                ...
                                {
                                    assetPath = Application.dataPath + AssetDatabase.GetAssetPath(tex)["Assets".Length..];
                    ...
                    MakeFromTransform((t, mesh, materials) =>
                    {
                        ...
                        Grendgine_Collada_Triangles[] triangles;
                        {
                            ...
                            for (int subMesh = 0; subMesh < mesh.subMeshCount; subMesh++)
                            {
                                ...
                                materialsDic.TryGetValue(materials[subMesh], out Grendgine_Collada_Material material);
                ...
                if (clips != null)
                {
                    ...
                    #region DisableOtherBehaviors
                    {
                        foreach (var comp in tmpObject.GetComponentsInChildren<Behaviour>(true))
                        {
                            ...
                            if (comp is Animator ||
                                ...
                                continue;
#if VOXELIMPORTER_ANIMATIONRIGGING
                            if (comp is VoxelImporterRigBuilder || comp is RigBuilder ||
                                comp is VoxelImporterRig || comp is Rig)
                                continue;
#endif
                            comp.enabled = false;
                        }
                    ...
                    try
                    {
                        ...
                        if (animator != null || animation != null)
                        {
                            ...
                            {
                                Transform FindPathTransform(Transform t, string p)
                                {
                            ...
                            foreach (var clip in clips)
                            {
                                ...
                                if (animator != null)
                                {
...
#if VOXELIMPORTER_ANIMATIONRIGGING
                                    VoxelImporterRigBuilder vaRigBuilder;
                                    ...
                                    #region BuildPlayableGraph
                                    {
                                        ...
                                        {
                                            vaRigBuilder = tmpObject.GetComponent<VoxelImporterRigBuilder>();
                                    ...
                                    for (int i = 0; i < frameTimes.Length; i++)
                                    {
                                        ...
                                        for (int j = 0; j < tmpTransforms.Length; j++)
                                        {
                                            ...
                                            tmpTransformMatrixs[j, i] = Matrix4x4.TRS(position ?? tmpTransforms[j].localPosition,
                                                                                        rotation ?? tmpTransforms[j].localRotation,
                                                                                        scale ?? tmpTransforms[j].localScale);
                                        }
                                    }
