...
[Support]
forum: https://discussions.unity.com/t/voxel-importer/702864
mail: support@alonesoft.sakura.ne.jp
twitter: @AlSoSupport
...
[Update History]
Version 1.4.3
- FIX: ScriptedImporter: Fixed an error that occurred when enabling Export in Unity 6 and later.
- FIX: Fixed a warning that appeared in Unity 6000.4 and later.

Version 1.4.2
- FIX : Fixed the process of getting the current RenderPipeline
- FIX : DaeExpolter : Latest update

Version 1.4.1
- FIX : Fixed an issue where an exception would occur when destroying a LowPolyTextureBuffer.

Version 1.4.0
- ADD : Unity6  : support
- FIX : Change the minimum operation version to Unity2021.3
- FIX : Edit : Fixed an issue where the Unity Menu was displayed in edit mode when the original model was displayed with the Space key.
- FIX : Material : Fixed material settings read and set from vox.
- FIX : Prefab : Fixed an issue where materials changed during the process of making a Prefab were initialized.
- FIX : Explosion : Preview: Supported operation in Prefab Mode, playback speed fixed.
- FIX : Explosion : BuiltinRP: Supported switching by recommending the use of the Shader Graph version as legacy, with the Surface Shader version as legacy.
- FIX : Explosion : Fixed material settings, fixed Color space support.


Version 1.3.4
- FIX : Reduced memory usage during import
- FIX : Delete old processes from Unity 2019.3 and earlier

Version 1.3.3
- ADD : Unity2023.2 : support
- ADD : ScriptedImporter : Added output to the console when an exception occurs.
- ADD : Optimize : Ignore Cavity : Changed the default setting from enabled to disabled
- FIX : Optimize : Ignore Cavity : Calculations required when enabled will not be performed when disabled.
